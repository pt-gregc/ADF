...
<cfscript>
	...
	variables.SELECT_BOX_WIDTH = 260;
	...
	variables.SHOW_EDIT_DELETE_LINKS = true;  	// Boolean - 'false' Disables both Edit and Delete options, 'true' Enables the SHOW_EDIT_LINKS and SHOW_DELETE_LINKS options
...
<cffunction name="loadEditDeleteItemLinks" access="public" returntype="string" hint="General Chooser - HTML for the Edit and Delete Item Links">
	...
	<cfscript>
		...
		if ( variables.SHOW_EDIT_DELETE_LINKS AND !arguments.readonly ) {
			...
			retItemsLinksHTML = "<table border=0><tr>";
			
			// Build the ITEM Edit button
		    if ( variables.SHOW_EDIT_LINKS ) {
		    	retItemsLinksHTML = retItemsLinksHTML & "<td>";
		   	 	retItemsLinksHTML = retItemsLinksHTML & loadEditItemLink(fieldName=arguments.fieldName,bean=arguments.bean,method=editMethod,formid=arguments.formid,csPageID=arguments.csPageID,readonly=arguments.readonly);
		    	editDeleteLinks = retItemsLinksHTML & "</td>";
		    }
		    
		    // Build the ITEM Delete button
		    if ( variables.SHOW_DELETE_LINKS ) {
		    	retItemsLinksHTML = retItemsLinksHTML & "<td>";
		   		retItemsLinksHTML = retItemsLinksHTML & loadDeleteItemLink(fieldName=arguments.fieldName,bean=arguments.bean,method=deleteMethod,formid=arguments.formid,csPageID=arguments.csPageID,readonly=arguments.readonly);
		    	retItemsLinksHTML = retItemsLinksHTML & "</td>";
		    }
		    
		    retItemsLinksHTML = retItemsLinksHTML & "</tr></table>";
		}
	
		return retItemsLinksHTML;
	</cfscript>
</cffunction>
...
<cffunction name="loadChooserInstructions" access="public" returntype="string" hint="General Chooser - Loads the instructions text for the Chooser field">
	...
	<cfif StructKeyExists(variables,"SHOW_INSTRUCTIONS") AND variables.SHOW_INSTRUCTIONS>
		...
		for ( i=1; i LTE ArrayLen(ceDataArray); i=i+1) {
			...
			if ( StructKeyExists(ceDataArray[i].Values, "#variables.DISPLAY_FIELD#") 
				...
				if ( variables.SHOW_EDIT_DELETE_LINKS AND !arguments.readonly ) {
				    ...
				    editDeleteLinks = "<div>"
				    editDeleteLinks = editDeleteLinks & loadEditDeleteItemLinks(fieldName=arguments.fieldID,formid=ceDataArray[i].formID,csPageID=ceDataArray[i].pageID,readonly=arguments.readonly);
