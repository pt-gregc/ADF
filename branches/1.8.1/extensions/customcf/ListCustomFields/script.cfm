...
<cfoutput>
	<script type="text/javascript">
		...
		function saveForm(){
			...
			jQuery("##success").hide();
			jQuery("##failure").hide();
			// make the call to update the fieldType
			...
			// if the data should be updated do that
			if( jQuery("##fixData").checked() )
			{
				jQuery.post("#application.ADF.ajaxProxy#",
				{
					bean: "utils_1_0",
					method: "updateListItems",
					fieldType: jQuery("##type").attr("value")
				},function (results){
					// nothing to do here
				}
			);
	...
	<form id="fieldType">
		...
		<fieldset>
			...
			<label for="fixData">Fix Data?</label> <input type="checkbox" name="fixData" id="fixData" />
