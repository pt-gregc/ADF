...
/* 2011-07-26- GAC - Modified - Added allDay parameter and updated the iCal, Google, and Yahoo APIs 
 * 								to handle all day events
 */
...
function iCalendar() {
	this._defaults = {
		...
		allday: 0,  // Is the event and allday event
...
$.extend(iCalendar.prototype, {
	...
	_updateICalendar: function(target, settings) {
		...
		var addSite = function(site, calId) {
			var inserts = {t: encodeURIComponent(settings.title),
				...
				s: ( settings.allday ? $.icalendar.formatDate(settings.start) : $.icalendar.formatDateTime(settings.start) ),
				e: ( settings.allday ? $.icalendar.formatDate(settings.end) : $.icalendar.formatDateTime(settings.end) ),
			...
			var url = site.url;
			if ( settings.allday && (site.display == 'Yahoo') ){  
				url = url.replace(/&amp;dur={p}/i,""); // /&amp;dur={p}/i
			}
	...
	_echo: function(target, calId) {
		...
		if (settings.echoUrl) {
			var urlArray = settings.echoUrl.split("?");
...
function makeICalendar(event) {
	...
	return 'BEGIN:VCALENDAR\r\n' +
		...
		( event.allday ? 
		'DTSTART;VALUE=DATE:' + $.icalendar.formatDate(event.start) 
		: 
		'DTSTART:' + $.icalendar.formatDateTime(event.start) ) + '\r\n' +
   		( event.allday ? 
		'DTEND;VALUE=DATE:' + $.icalendar.formatDate(event.end)
		:
