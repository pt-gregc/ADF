...
--->
<cfcomponent name="general_chooser" extends="ADF.lib.ceData.ceData_3_0">

<cfproperty name="version" value="2_1_0">
...
History:
	...
	2016-12-06 - GAC - Added a _loadStyles_custom() custom override function
--->
<cffunction name="loadStyles" access="public" returntype="string" output="true" hint="">
	...
	<cfscript>
		var retInitHTML = "";
		var itemCellRightHeight = "";
		var gcCustomStyles = "";

		if ( StructKeyExists(variables,"SELECT_ITEM_HEIGHT") AND IsNumeric(variables.SELECT_ITEM_HEIGHT) )
			itemCellRightHeight = (variables.SELECT_ITEM_HEIGHT-4);
		...
		if ( variables.SHOW_EDIT_DELETE_LINKS  AND (variables.SHOW_EDIT_LINKS OR variables.SHOW_DELETE_LINKS) AND IsNumeric(variables.SELECT_ITEM_HEIGHT) )
	...
	<cfsavecontent variable="retInitHTML">
		<cfoutput>
			...
			<style>
				...
				<cfif IsNumeric(itemCellRightHeight)>
				...
				###arguments.fieldName#-sortable2 li { 
					margin: 5px; 
					padding: 5px;
				<cfif IsNumeric(variables.SELECT_ITEM_WIDTH)>
					width: #variables.SELECT_ITEM_WIDTH#px;
				</cfif>
				<cfif IsNumeric(variables.SELECT_ITEM_HEIGHT)>
				...
				<!--- // Render the Custom styles string  --->
				<cfset gcCustomStyles = _loadStyles_custom(argumentCollection=arguments)>
				#gcCustomStyles#
			</style>
		</cfoutput>
	</cfsavecontent>

	<!--- // IMPORTANT!! addHeaderCSS is called from inside the _RENDER.cfc renderControl() function --->
...
Name:
	$_loadStyles_custom
Summary:
	An empty function that can be used for a site level override to add CUSTOM style definitions to the _loadStyles base function.
	
	Style definitions added to an override version of this function will be added 
	inside the _loadStyles() <style> block
Returns:
	string
Arguments:
	ARGS
History:
	2016-12-06 - GAC - Created
--->
<cffunction name="_loadStyles_custom" access="public" returntype="string" output="true" hint="">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="readonly" type="boolean" default="false" required="false">
	<cfargument name="gcCustomParams" type="struct" default="#getCustomGCparams()#" required="false">

	<cfscript>
		var cftGCheaderCustomCSS = '';
	</cfscript>

	<!--- No <style> tags needed ( provided by the _loadStyles base function ) --->
	<cfsavecontent variable="cftGCheaderCustomCSS">
		<cfoutput>
			<!--- // An empty function that can be used for a site level override to add CUSTOM style definitions to the _loadStyles base function --->
		</cfoutput>
	</cfsavecontent>

	<cfreturn cftGCheaderCustomCSS>
</cffunction>
...
History:
	...
	2016-12-05 - GAC - The loadAvailable and loadAvailableOptions to the arguments.inputParameters struct
						  -  Isolated the checkResizeWindow() JS function to a specific instance of the general chooser field
	2016-12-06 - GAC - Broke the chooser JS block in to section for easier overriding of specific sections.
	                 - Added a custom renderJS block for adding additional custom JS
--->
<cffunction name="renderChooserJS" access="public" returntype="void" output="true" hint="Renders the Chooser CFT's JavaScript.">
	...
	<cfscript>
		...
		if ( NOT StructKeyExists(arguments.inputParameters,"loadAvailable")  )
			arguments.inputParameters.loadAvailable = 0;
		if( NOT StructKeyExists(arguments.inputParameters, "loadAvailableOption") )
			arguments.inputParameters.loadAvailableOption = "useServerSide"; 	//	useServerSide or useJavascript
		...
		// Render the General Chooser Javascript functions 
		savecontent variable="cftGeneralChooserFooterJS" {
		
			WriteOutput('<script type="text/javascript">');
			WriteOutput('//<!--');
			
			/* Include the Global Variables Javascript block */
			_renderChooserJS_globalVars(argumentCollection=arguments);
			
			/* Include the jQuery Document Ready Javascript block */
			_renderChooserJS_docReady(argumentCollection=arguments);
			
			/* Include the Render Chooser Items Javascript block */
			_renderChooserJS_chooserItems(argumentCollection=arguments);
			
			/* Include the Enable Drag and Drop Javascript block */
			_renderChooserJS_dragAndDrop(argumentCollection=arguments);
			
			/* Include the Dialog and Window Resize Javascript block */
			_renderChooserJS_dialog(argumentCollection=arguments);
			
			/* Include the Form Callback Javascript block */
			_renderChooserJS_formCallback(argumentCollection=arguments);
			
			/* Include the Form Edit Callback Javascript block */
			_renderChooserJS_formEditCallback(argumentCollection=arguments);
			
			/* Include the Field Validate Javascript block */
			_renderChooserJS_fieldValidate(argumentCollection=arguments);
			
			/* Include the Utility Javascript block */
			_renderChooserJS_utility(argumentCollection=arguments);
			
			/* Include the Custom Javascript block */
			_renderChooserJS_custom(argumentCollection=arguments);
		...
		application.ADF.scripts.addFooterJS(cftGeneralChooserFooterJS,"SECONDARY");
	</cfscript>
</cffunction>

<!--- ******************************************* --->
<!--- ** START - _renderChooserJS Subfunctions ** --->
<!--- ******************************************* --->

<!---
	_renderChooserJS_globalVars
--->
<cffunction name="_renderChooserJS_globalVars" access="public" returntype="void" output="true" hint="Renders the form callback section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
<!--- // No <script> tags needed ( provided by the _renderChooserJS() base function ) --->
<cfoutput>
...
<!---
	_renderChooserJS_docReady
--->
<cffunction name="_renderChooserJS_docReady" access="public" returntype="void" output="true" hint="Renders the form callback section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
jQuery(function(){
	...
	#arguments.fieldName#_checkResizeWindow();

	<cfif arguments.inputParameters.loadAvailable>
...
<!---
	_renderChooserJS_chooserItems
--->
<cffunction name="_renderChooserJS_chooserItems" access="public" returntype="void" output="true" hint="Renders the render chooser UI section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
<!---
	_renderChooserJS_dragAndDrop
--->
<cffunction name="_renderChooserJS_dragAndDrop" access="public" returntype="void" output="true" hint="Renders the render chooser UI section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
<!---
	_renderChooserJS_dialog
--->
<cffunction name="_renderChooserJS_dialog" access="public" returntype="void" output="true" hint="Renders the dialog/window section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
// A Utility function to checkResize the window function
function #arguments.fieldName#_checkResizeWindow()
{
...
<!---
	_renderChooserJS_formCallback
--->
<cffunction name="_renderChooserJS_formCallback" access="public" returntype="void" output="true" hint="Renders the form callback section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
<!---
	_renderChooserJS_formEditCallback
--->
<cffunction name="_renderChooserJS_formEditCallback" access="public" returntype="void" output="true" hint="Renders the form edit callback section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
<!---
	_renderChooserJS_fieldValidate
--->
<cffunction name="_renderChooserJS_fieldValidate" access="public" returntype="void" output="true" hint="Renders the utility section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
<!---
	_renderChooserJS_utility
--->
<cffunction name="_renderChooserJS_utility" access="public" returntype="void" output="true" hint="Renders the utility section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
Author: 	
	PaperThin, Inc.
Name:
	$_renderChooserJS_custom
Summary:
	An empty function that can be used for a site level override to add CUSTOM General Chooser Javascript functions.
	
	Script functions added to an override version of this function will be added 
	inside the_renderChooserJS <script> block
Returns:
	string
Arguments:
	ARGS
History:
	2016-12-06 - GAC - Created
--->
<cffunction name="_renderChooserJS_custom" access="public" returntype="void" output="true" hint="Renders the form callback section of the Chooser CFT's JavaScript.">
	<cfargument name="fieldName" type="string" required="true">
	<cfargument name="formname" type="string" required="true">
	<cfargument name="currentValue" type="string" default="" required="false">
	<cfargument name="readonly" type="boolean" default="false" required="false">
...
<!--- ******************************************* --->
<!--- **  END - _renderChooserJS Subfunctions  ** --->
<!--- ******************************************* --->
